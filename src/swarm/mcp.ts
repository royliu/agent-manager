...
const GM_TOOLS: Tool[] = [
  ...
  { name: 'task_update', description: 'Change a task: title, description, priority, eta, dependsOn, planFirst, useWorktree, reviewBy, hold. hold=true parks a task so the task manager does not start it when an agent frees up; hold=false (or task_start) releases it.', inputSchema: S({ id: ID, title: s(''), description: s(''), priority: n(''), eta: ETA, dependsOn: { type: 'array', items: { type: 'number' } }, planFirst: b(''), useWorktree: b(''), reviewBy: { type: 'string', enum: ['gm', 'user'] }, hold: b('park the task (true) or release it (false)'), by: s('') }, ['id']) },
  { name: 'task_start', description: 'Start work on a task now: on the named agent, or the first idle one, ahead of anything the task manager would pick by itself. Waits automatically while dependencies are unfinished. Also releases a task on hold.', inputSchema: S({ id: ID, agent: s('agent name, optional') }, ['id']) },
  ...
  { name: 'models_set', description: 'Change which model a group runs on, for this team only, and only when the owner asks. Groups: gm (you), taskManager, agents (on Claude Code profiles), codexAgents (on Codex profiles). Give an official model id (claude-fable-5-1, claude-opus-5, claude-sonnet-5; for Codex profiles the id Codex accepts, e.g. gpt-5.6-sol), or "default" to go back to the profile\'s own model. Short aliases such as "opus" work but float to whichever version the tool currently maps them to. Your new model applies the next time the owner opens this conversation; the task manager\'s at its next answer; the agents\' at their next run.', inputSchema: S({ gm: s('model for you'), taskManager: s('model for the task manager'), agents: s('model for task agents on Claude Code profiles'), codexAgents: s('model for task agents on Codex profiles') }) },
  { name: 'team_profile_set', description: "Change which profile (subscription) the task agents run on, for this team only, and only when the owner asks. Any Claude Code or Codex profile the owner has; \"default\" means the same profile as you. Idle agents move now; an agent in the middle of a run is named so the owner can move it later. The task manager always stays on your profile.", inputSchema: S({ profile: s('profile name, or "default"') }, ['profile']) },
...
export async function runMcpBridge(opts: { swarm: string; role: 'gm' | 'agent'; agent?: string; task?: number }): Promise<void> {
  ...
  async function call(name: string, a: Json): Promise<Json> {
    ...
    switch (name) {
      ...
      case 'team_profile_set': {
        const r = await client.call<{ profile: string; moved: string[]; busy: string[] }>('profiles.set', { agents: a.profile, by: gm });
        out = `Task agents now run on profile ${r.profile}.${r.moved.length ? ` Moved: ${r.moved.join(', ')} (each starts a fresh session there on its next task).` : ''}${r.busy.length ? ` Still working, move later with am agent move: ${r.busy.join(', ')}.` : ''}`;
        break;
      }
      case 'models_set': {
